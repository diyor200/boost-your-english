...
# new collection
@quiz_router.message(Command("new_collection"))
...
@quiz_router.message(Collection.Title)
...
@quiz_router.message(Command("new_collection_word"))
async def new_collection(message: types.Message, state: FSMContext):
...
@quiz_router.message(CollectionWord.Collection)
async def new_word(message: types.Message, state: FSMContext):
...
@quiz_router.message(CollectionWord.Word)
async def new_word(message: types.Message, state: FSMContext):
    file = await message.bot.get_file(message.document.file_id)
    f = await message.bot.download_file(file.file_path)
    workbook = load_workbook(f)
